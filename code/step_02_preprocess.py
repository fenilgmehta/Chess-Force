...
def load_pgn(pgn_file_path: Union[str, Path]) -> Union[IO, None]:
    ...
    print(f"DEBUG: pgn file {pgn_file_path} successfully loaded")
    return pgn
...
def preprocess_pgn(pgn_file_path: Union[str, Path], output_path: Union[str, Path]):
    def savepoint(game_count, file_count):
        # (last game written + 1), (next file number to be written)
        os.system(f"echo '{game_count},{file_count}' > z_game_num.txt")

    def readpoint() -> List[int]:
        if not Path("z_game_num.txt").exists():
            return [1, 1]
        return [int(i) for i in subprocess.getoutput("cat z_game_num.txt").split(",")]
    ...
    for i in iterate_pgn(pgn_file):
        ...
        print(f"DEBUG: processing game_count = {game_count}")
        board_count = 1
        ...
        if len(res_pd) > BATCH_SIZE:
            ...
            print(f"DEBUG: boards successfully written to file: {output_file}")
...
if __name__ == "__main__":
    preprocess_pgn(pgn_file_path="KingBase2019-A80-A99.pgn", output_path="./")
